...
def connect():
    ...
    for ssid, password in networks:
        ...
        for i in range(wifiTries):
            if wlan.isconnected():
                scrollMessage(f"Conn!", 0.3, 0.001)
            ...
            error = False
            scrollMessage(f"Conn {ssid} {i+1}/{wifiTries}", 0.06, 0.001)
            time.sleep(1)
    error = True
    message = "No WiFi"
...
def showBattery():
    ...
    time.sleep(1)
    while not picounicorn.is_pressed(picounicorn.BUTTON_B):
        percentage = (ina219.getBusVoltage_V() - 3)/1.2*100
        if(percentage<0):
            percentage=0
        elif(percentage>100):
            percentage=100
        ...
        scrollMessage(message, 0.06, 0.02)
        print(message)
